...
public class VersionRepo(NotesDatabaseContext context) : IVersionRepo {
    ...
    public async Task<SyncVersion[]> GetChangedVersions(string vaultId, long sinceCursor, bool includePayloads) {
        ...
        if (includePayloads) {
            ...
            return rows.Select(v => Project(v, v.Payload)).ToArray();
        }

        // Written out rather than sharing Project() because this one has to be translated to SQL:
    ...
    /// <summary>Maps a stored row to what sync sends, once the row is already in memory.</summary>
    private static SyncVersion Project(NoteVersion v, string? payload) {
